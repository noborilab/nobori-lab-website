...
function VideoCard({ v, className = '' }) {
  return (
    <a
      href={`https://youtu.be/${v.id}`}
      target="_blank"
      rel="noopener noreferrer"
      className={`group rounded-xl border border-border bg-bg overflow-hidden hover:-translate-y-1 hover:shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-all duration-300 ${className}`}
    >
...
function PodcastCard({ ep, className = '' }) {
  return (
    <a
      href={ep.href}
      target="_blank"
    ...
    <section id="talks" className="py-24 bg-bg px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-14">
          <TypewriterLabel text="Online Talks" />
          <button
            onClick={() => setViewAll(!viewAll)}
            className="font-mono text-[13px] uppercase tracking-[0.1em] px-4 py-1.5 rounded-full border border-border text-text/40 hover:text-navy hover:border-navy/40 transition-all"
          >
          ...
          {viewAll ? (
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                {podcasts.map((ep) => (
                  <PodcastCard key={ep.title} ep={ep} />
                ))}
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              >
                {videos.map((v) => (
                  <VideoCard key={v.id} v={v} className="shrink-0 w-[300px] sm:w-[320px] snap-start" />
                ))}
                {podcasts.map((ep) => (
                  <PodcastCard key={ep.title} ep={ep} className="shrink-0 w-[300px] sm:w-[320px] snap-start" />
                ))}
              </div>
